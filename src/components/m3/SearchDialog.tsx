...
const SearchDialog = forwardRef<HTMLDivElement, SearchDialogProps>(
  function SearchDialog({ isOpen, onClose, entries, onSelectEntry }, _ref) {
    ...
    useEffect(() => {
      ...
      const onKey = (e: KeyboardEvent) => {
        ...
        else if (e.key === "Enter" && results[selectedIndex]) handleSelect(results[selectedIndex].entry);
      };
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{ borderBottom: '1px solid hsl(var(--md-sys-color-outline-variant) / 0.4)' }}
                >
                ...
                <div className="max-h-[min(55vh,420px)] overflow-y-auto">
                  {query.trim() && results.length === 0 ? (
                    ...
                    <div className="py-1">
                      <p className="px-5 py-2 label-small text-md-on-surface-variant">
                      ...
                      {results.slice(0, 10).map((r, i) => (
                        <button
                          ...
                          style={{
                            background: i === selectedIndex ? 'hsl(var(--md-sys-color-secondary-container))' : undefined,
                          }}
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {recentSearches.map((q) => (
                              <button key={q} onClick={() => handleRecentClick(q)} className="md-chip">{q}</button>
                            ))}
                          </div>
                        </div>
                      )}
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {["dark mode", "security", "2.5.0", "performance", "breaking"].map((s) => (
                            <button key={s} onClick={() => handleRecentClick(s)} className="md-chip">{s}</button>
                          ))}
                        </div>
                      </div>
                    </div>
